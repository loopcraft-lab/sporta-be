...
const ModeratorModule = ['AUTH', 'BUSINESS', 'SPORT', 'MEDIA']
const ClientModule = ['AUTH', 'SPORT', 'SPORTPROFILE', 'MEDIA']
const OwnerModule = ['AUTH', 'BUSINESS', 'SPORT', 'SPORTPROFILE', 'MEDIA']
...
async function bootstrap() {
  ...
  const ownerPermissionIds = updatedPermissionsInDb
    .filter((item) => OwnerModule.includes(item.module))
    .map((item) => ({ id: item.id }))
  const moderatorPermissionIds = updatedPermissionsInDb
    .filter((item) => ModeratorModule.includes(item.module))
    .map((item) => ({ id: item.id }))
  ...
  await Promise.all([
    ...
    updateRole(ownerPermissionIds, RoleName.Owner),
    updateRole(moderatorPermissionIds, RoleName.Moderator),
